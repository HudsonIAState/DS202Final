...
df <- data.frame(
  ...
  chars2 = unlist(meaning_list),
  stringsAsFactors = FALSE
)

# Sort the data frame based on 'doubles' column in descending order
sorted_df <- df[order(-df$doubles), ]

head(new_cols_data[[i]])
abo <- question_data %>%
      filter(as.vector(.[[cols_data[1]]]) == new_cols_data[[1]][1])
print(cols_data[1])
print(new_cols_data[[1]][1])
ab111 <- mean(filtered_data$Data_Value)
```
